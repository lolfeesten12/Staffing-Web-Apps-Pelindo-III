            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Pengesahan</li>
                    </ol>
                </nav>
            </div>
        </div>
                                ...
                                <table id="example" class="table table-striped table-bordered dataTable"
                                    ...
                                    <thead>
                                        <tr role="row">
                                            ...
                                            <th class="sorting" tabindex="0" aria-controls="example" rowspan="1"
                                            colspan="1" aria-label="Position: activate to sort column ascending"
                                            style="width: 40px;">Unit Kerja</th>
                                            @else
                                    ...
                                    <tbody>
                                        ...
                                        <tr role="row" class="odd">
                                            ...
                                            <td class="text-center">
                                                <a href="{{ route('approval-penilaian.show',$item->id_penilaian) }}"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header bg-light-success text-success">
                <h5 class="modal-title" id="exampleModalCenterTitle">Pengesahan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('approval-penilaian-status', $tes->id_penilaian) }}?status_acc=Disahkan" method="POST" class="d-inline">
                @csrf
                <div class="modal-body">
                    <div class="form-group">Apakah Anda Yakin Mengesahkan Penilaian Pegawai Atas Nama {{ $item->Pegawai->nama_pegawai }} ?</div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button class="btn btn-success" type="submit">Ya! Sahkan</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="ModalTolak-{{ $item->id_penilaian }}" data-backdrop="static" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('approval-penilaian-status', $item->id_penilaian) }}?status_acc=Tidak Disahkan" method="POST" class="d-inline">
                @csrf
                <div class="modal-body">
                    <div class="form-group">Apakah Anda Yakin Tidak Mengesahkan Penilaian Pegawai Atas Nama {{ $item->Pegawai->nama_pegawai }} ?</div>
                </div>
                <div class="modal-footer ">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button class="btn btn-danger" type="submit">Ya! Tolak Pengesahan</button>
                </div>
            </form>
        </div>
    </div>
