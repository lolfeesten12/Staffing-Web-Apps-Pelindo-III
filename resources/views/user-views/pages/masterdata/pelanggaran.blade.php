                                    ...
                                    <thead>
                                        <tr role="row">
                                            ...
                                            <th class="sorting" tabindex="0" aria-controls="example" rowspan="1"
                                                ...
                                                style="width: 210px;">Nama Pelanggaran</th>
                                                <th class="sorting" tabindex="0" aria-controls="example" rowspan="1"
                                                colspan="1" aria-label="Position: activate to sort column ascending"
                                                style="width: 90px;">Sanksi</th>
                                            <th class="sorting" tabindex="0" aria-controls="example" rowspan="1"
            ...
            <form action="{{ route('pelanggaran.store') }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label class="small mb-1 mr-1" for="pelanggaran">Nama Pelanggaran</label><span class="mr-4 mb-3"
                            style="color: red">*</span>
                        <input class="form-control" name="pelanggaran" type="text" id="pelanggaran"
                            placeholder="Input Nama Sanksi" value="{{ old('pelanggaran') }}" required />
                    </div>
                    <div class="form-group">
                        <label class="small mb-1 mr-1" for="id_sanksi">Sanksi yang didapat</label><span
                            class="mr-4 mb-3" style="color: red">*</span>
                        <select class="form-select" name="id_sanksi" id="id_sanksi"
                            value="{{ old('id_sanksi') }}" required>
                            <option>Pilih Sanksi</option>
                            @foreach ($sanksi as $item)
                            <option value="{{ $item->id_sanksi }}">{{ $item->nama_sanksi }}
                            </option>
                            @endforeach
                        </select>
                    </div>
...
<div class="modal fade" id="Modaledit-{{ $item->id_pelanggaran }}" data-backdrop="static" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('pelanggaran.update', $item->id_pelanggaran) }}" method="POST">
                @method('PUT')
                @csrf
                <div class="modal-body">
                    <label class="small mb-1">Isikan Form Dibawah Ini</label>
                    ...
                    <div class="form-group">
                        <label class="small mb-1 mr-1" for="pelanggaran">Nama Pelanggaran</label><span class="mr-4 mb-3"
                            style="color: red">*</span>
                        <input class="form-control" name="pelanggaran" type="text" id="pelanggaran"
                            placeholder="Input Nama Sanksi" value="{{ $item->pelanggaran }}" required />
                    </div>
                    <div class="form-group">
                        <label class="small mb-1 mr-1" for="id_sanksi">Sanksi yang didapat</label><span
                            class="mr-4 mb-3" style="color: red">*</span>
                        <select class="form-select" name="id_sanksi" id="id_sanksi"
                            value="{{ $item->Sanksi->id_sanksi }}" required>
                            <option value="{{ $item->Sanksi->id_sanksi }}">{{ $item->Sanksi->nama_sanksi }}</option>
                            @foreach ($sanksi as $item)
                            <option value="{{ $item->id_sanksi }}">{{ $item->nama_sanksi }}
                            </option>
                            @endforeach
...
<div class="modal fade" id="Modalhapus-{{ $item->id_pelanggaran }}" data-backdrop="static" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content bg-danger">
            ...
            <form action="{{ route('pelanggaran.destroy', $item->id_pelanggaran) }}" method="POST" class="d-inline">
                @csrf
                @method('delete')
                <div class="modal-body text-white">
                    Apakah Anda Yakin Menghapus Data Master Pelanggaran {{ $item->pelanggaran }} ?</div>
                <div class="modal-footer">
